 ...
 *                    one entry for each Enum value. Each item in the list
...
function validateEnumParams(name, values, properties) {
    ...
    values.forEach((value) => {
        assert(isNonEmptyString(value), "Value '" + value + "' was not a "
                + "non-empty string.");
    ...
    for (let i = 0; i < properties.length; i++) {
        ...
        assert(Array.isArray(valueProps), "Property list for Enum value " + i
                + " is not an array.");
        if (i > 0) {
            const expectedKeys = Object.key(propertyKeys).length;
...
module.exports = {
     ...
     *                     the Enum type. See the validateEnumParams
     *                     function's Javadoc for the required format of this
     *                     parameter.
     *
    ...
    createEnum : function(name, values, properties) {
        try {
            ...
            Error("Unable to create '" + name + "' enum type: "
                    + err.message);
        }
        let enumObject = {};
