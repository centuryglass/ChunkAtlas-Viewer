...
function loadAllMapKeys() {
    ...
    return fetch(request).then(response => response.json())
        .then(keyArray => {
            const keys = {};
            MapTypeEnum.forEach(mapType => keys[mapType] = new MapKey());
            ...
            keyArray.forEach(key => {
                assert(isDefined(key) && key !== null
                        ...
                        && isDefined(key.type),
                        "loadAllMapKeys: received invalid map key " + key);
                var mapType;
                for (let i = 1; i <= MapTypeEnum.count; i++) {
                    if (key.type === MapTypeEnum.properties[i].name) {
                        mapType = i;
                        break;
                    }
                }
                if (! isDefined(mapType)) {
                    if (! unhandledKeys.includes(key.type)) {
                        unhandledKeys.push(key.type);
                    }
                    return;
                }
                let imageOrColor = "#000000";
                if (isDefined(key.icon_url)) {
                    imageOrColor = key.icon_url;
                }
                else if (isDefined(key.color)) {
                    imageOrColor = "#" + key.color;
                }
                keys[mapType].addValue(key.description, imageOrColor);
            });
            unhandledKeys.forEach(type => {
                    console.log(type + " keys not handled.")
                    });
            return keys;
        });
